			...
			<div v-if="active === 2">
				<el-form-item label="操作系统" prop="os_name">
					<el-radio-group v-model="ruleForm.os_name">
						<el-radio v-for="(item,index) in ostype" 
						:key="index" :label="item.name" :value="item.name"></el-radio>
					</el-radio-group>
				</el-form-item>
			...
			<div v-if="active === 3">
				<el-card shadow="never">
					<ul>
						...
						<li><span>房间</span> {{ ruleForm.room }}</li>
						<li><span>位置</span> {{ ruleForm.location_name }}</li>
						<li><span>类型</span>{{ ruleForm.type_name }}</li>
					</ul>
				</el-card>
			</div>
			<div v-if="active === 4">
